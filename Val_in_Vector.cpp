...
int main()
{
  ...
  /* Your solution goes here  */
  maxMiles = milesTracker[0][0]; // Initialize with the first element
  minMiles = milesTracker[0][0]; // Initialize with the first element
  ...
  for (i = 0; i < NUM_ROWS; i++)
  {
    for (j = 0; j < NUM_COLS; j++)
    {
      ...
      {
        maxMiles = milesTracker[i][j]; // Update maxMiles if a greater value is found
      }
      if (milesTracker[i][j] < minMiles)
      {
        minMiles = milesTracker[i][j]; // Update minMiles if a smaller value is found
      }
    }
  }
